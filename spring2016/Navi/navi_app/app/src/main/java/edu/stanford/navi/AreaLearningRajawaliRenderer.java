...
/**
 * This class implements the rendering logic for the Area Description example using Rajawali.
 */
    ...
    protected void onRender(long ellapsedRealtime, double deltaTime) {
        ...
        synchronized (this) {
            if (mPoseUpdated) {
                ...
                if (mIsRelocalized) {
                    if (mGreenTrajectory.getLastPoint().distanceTo2(mDevicePose.getPosition()) >
                            THRESHOLD) {
                        mGreenTrajectory.addSegmentTo(mDevicePose.getPosition());
                    }
                } else {
                    if (mBlueTrajectory.getLastPoint().distanceTo2(mDevicePose.getPosition()) >
                            THRESHOLD) {
                        mBlueTrajectory.addSegmentTo(mDevicePose.getPosition());
                    }
                }
                mTouchViewHandler.updateCamera(mDevicePose.getPosition(), mDevicePose
                        .getOrientation());
            }
        }
