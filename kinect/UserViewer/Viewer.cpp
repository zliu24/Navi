...
void SampleViewer::display()
{
    ...
    Mat RGBFrame, DepthFrame, DepthFrameShow;
         ...
         m_eViewState == DISPLAY_MODE_IMAGE) && m_colorFrame.isValid())
    {
        ...
        const openni::RGB888Pixel* RGBBuffer = (const openni::RGB888Pixel*)m_colorFrame.getData();
        RGBFrame = Mat(m_colorFrame.getHeight(), m_colorFrame.getWidth(), CV_8UC3, (void *)RGBBuffer);
         ...
         m_eViewState == DISPLAY_MODE_DEPTH) && m_depthFrame.isValid())
    {
        ...
        const openni::DepthPixel* DepthBuffer = (const openni::DepthPixel*)m_depthFrame.getData();
        DepthFrame = Mat(m_depthFrame.getHeight(), m_depthFrame.getWidth(), CV_16U, (void *)DepthBuffer);
        DepthFrame.convertTo(DepthFrameShow, CV_8U, 1.0/256);
        equalizeHist(DepthFrameShow, DepthFrameShow);
    }
    
    if (!RGBFrame.empty() && !DepthFrame.empty()) {
        procFrame(RGBFrame, DepthFrame);
        imshow("RGB", RGBFrame);
        imshow("Depth", DepthFrameShow);
    }
